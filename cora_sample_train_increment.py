...
mask = [False for i in range(new_y_train.shape[0])]
...
if __name__ == "__main__":
    ...
    for i in range(18):
        ...
        print(f"{comb_train[mask].shape=}")

        main_train(DataLoader(comb_train[mask],batch_size=batch_size,shuffle=True,num_workers=0),DataLoader(comb_val,batch_size=batch_size,shuffle=True,num_workers=0),i+1)
        accuracy, csr = main_test(DataLoader(comb_test,batch_size=batch_size,shuffle=False,num_workers=0), modelname=f'model_{i+1}.pkl')

        samples.append(comb_train[mask].shape[0])
